...
public class MainActivity extends Activity implements OnClickListener{
	...
	private Calculadora_Logica calc;  //objeto para la logica de la calculadora
	                                  //(operaciones)
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		//el campo de texto inicia con el numero 0
		etCampoNumerico.setText("0");
		
		calc = new Calculadora_Logica();  //instancia para la logica de la calculadora
	}
	...
	//accion al presionar un boton (implementa interfaz de escucha OnClickListener())
	@Override
	public void onClick(View v) {
		//Introduccion de un numero "n". Si hay un cero en el campo lo sustituimos 
		//por n, sino lo cancatenamos con el numero (distinto de cero) que hay en el campo
		//en el caso de que lo ultimo presionado fue un boton de numero.
		//Si lo ultimo presionado fue operacion escribimos n en el campo.
		if ((String)v.getTag() == COMANDO_NUMERO){
			...
			if ((String)v.getTag() == COMANDO_CAMBIAR_SIGNO){
				//si el n�mero en el EditText inicia con el signo "-"
				//cambiamos su signo
				if (!etCampoNumerico.getText().toString().equalsIgnoreCase("0")){
					...
					}else{  //o concatenamos "-" al principio del n�mero
		...
		/*
		//se deberia anidar (if else if) pues switch(String) no esta permitido para Android
		...
		if ((String)v.getTag() == COMANDO_CAMBIAR_SIGNO){
			//si el numero en el EditText inicia con el signo "-"
			//cambiamos su signo
			if (!etCampoNumerico.getText().equals("0")){
				if (etCampoNumerico.getText().toString().charAt(0) == '-'){
					etCampoNumerico.setText(etCampoNumerico.getText().toString().substring(1));
